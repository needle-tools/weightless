...
func TestSummaryTypeLabelsArePlainTableData(t *testing.T) {
	report := scan.Report{
		Summary: []scan.ProviderSummary{
			...
			{Provider: "unity", Artifacts: 1, BytesHuman: "4.0 GiB"},
		...
		Artifacts: []scan.Artifact{
			...
			{Category: categoryProjectCaches, PrimaryProvider: "unity", Path: "/projects/a"},
	...
	if rows[0][0] != "MODEL" || rows[1][0] != "VM" || rows[2][0] != "PROJECT" || rows[3][0] != "SESSION" {
		t.Fatalf("unexpected type labels: %#v", []string{rows[0][0], rows[1][0], rows[2][0], rows[3][0]})
	}
}
